...
from src.utils.api import API_BASE_URL, make_api_request
...
def _display_current_files():
    """Display current files list with remove buttons."""
        ...
        for i, file_info in enumerate(st.session_state.uploaded_files_list):
            ...
            with col2:
                file_type_icon = (
                    ...
                    or file_info["name"].lower().endswith(".pdf")
                    else "🖼️"
                )
...
def _display_processing_summary():
    """Display summary of files ready for processing."""
    pdf_count = sum(
        1
...
def _process_files(document_name):
    ...
    # Prepare form data
    form_data = {"name": document_name}

    with st.spinner("Processing files... This may take a while."):
        try:
            response = requests.post(
                f"{API_BASE_URL}/document", files=files_data, data=form_data
            )
...
def show_processing_page():
    ...
    st.write(
        "Add files one by one in the order you want them processed (top to bottom)."
    )

    # Display current files list
    _display_current_files()
